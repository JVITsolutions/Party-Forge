...
func resolve_bundle(packet: DamagePacket, target: CombatantAdapter) -> DAMAGE_BUNDLE:
	...
	for index: int in critical_flags.size():
		var instance_health: HealthComponent = captured_health if is_instance_valid(captured_health) else null
		...
		if result.killing_blow:
			target_alive = false
			total_overkill += result.excess_damage
			killing_event = event
			target_killed.emit(event.copy())
		elif result.overkill_only:
			total_overkill += result.excess_damage

	if killing_event != null:
		_overkill_buffer.record(target_id, total_overkill, {
