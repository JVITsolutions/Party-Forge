...
static func build_idle(action_id: StringName) -> Animation:
	...
	if action_id == &"idle":
		left_shoulder_base = Vector3(0.30, -0.05, 0.38)
		right_shoulder_base = Vector3(0.24, -0.16, -0.32)
		left_elbow_base = Vector3(0.42, 0.0, 0.18)
		right_elbow_base = Vector3(0.36, 0.0, -0.16)
...
static func build_walk(action_id: StringName) -> Animation:
	...
	_add_rotation_track(animation, LEFT_SHOULDER, _five(Vector3(0.30, -0.05, 0.38), Vector3(0.34, -0.05, 0.42), Vector3(0.26, -0.05, 0.34), Vector3(0.28, -0.05, 0.36), Vector3(0.30, -0.05, 0.38)), times)
	_add_rotation_track(animation, LEFT_ELBOW, _five(Vector3(0.42, 0, 0.18), Vector3(0.46, 0, 0.20), Vector3(0.40, 0, 0.16), Vector3(0.38, 0, 0.17), Vector3(0.42, 0, 0.18)), times)
	_add_rotation_track(animation, RIGHT_SHOULDER, _five(Vector3(0.24, -0.16, -0.32), Vector3(0.20, -0.16, -0.28), Vector3(0.27, -0.16, -0.36), Vector3(0.26, -0.16, -0.34), Vector3(0.24, -0.16, -0.32)), times)
	_add_rotation_track(animation, RIGHT_ELBOW, _five(Vector3(0.36, 0, -0.16), Vector3(0.32, 0, -0.14), Vector3(0.40, 0, -0.18), Vector3(0.38, 0, -0.17), Vector3(0.36, 0, -0.16)), times)
	return animation
...
static func _attack_pose_data(action_id: StringName) -> Dictionary:
	...
	match action_id:
		...
		&"paladin_hammer_smite":
			...
			_set_phases(data, LEFT_SHOULDER, Vector3(0.38, -0.06, 0.40), Vector3(0.46, 0, 0.44), Vector3(0.36, -0.04, 0.38))
		...
		&"ranger_quick_bow_shot":
			...
			_set_phases(data, RIGHT_ELBOW, Vector3(0.56, 0, -0.18), Vector3(0.40, 0, -0.08), Vector3(0.48, 0, -0.12))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.62, 0, 0.52), Vector3(0.40, 0, 0.24), Vector3(0.46, 0, 0.30))
			_set_phases(data, LEFT_ELBOW, Vector3(0.68, 0, 0.20), Vector3(0.50, 0, 0.12), Vector3(0.58, 0, 0.16))
		...
		&"marksman_heavy_bow_shot":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.38, -0.22, -0.40), Vector3(0.30, -0.10, -0.18), Vector3(0.34, -0.14, -0.26))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.66, 0, -0.22), Vector3(0.46, 0, -0.10), Vector3(0.56, 0, -0.16))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.78, 0, 0.64), Vector3(0.48, 0, 0.30), Vector3(0.58, 0, 0.38))
			_set_phases(data, LEFT_ELBOW, Vector3(0.76, 0, 0.24), Vector3(0.54, 0, 0.14), Vector3(0.64, 0, 0.18))
		...
		&"rogue_dagger_flurry":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.52, -0.12, -0.46), Vector3(0.30, 0, 0.46), Vector3(0.42, -0.08, -0.20))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.34, -0.06, 0.46), Vector3(0.52, 0, -0.34), Vector3(0.38, -0.04, 0.28))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.64, 0, -0.18), Vector3(0.38, 0, 0.18), Vector3(0.52, 0, -0.10))
			_set_phases(data, LEFT_ELBOW, Vector3(0.54, 0, 0.18), Vector3(0.68, 0, -0.16), Vector3(0.58, 0, 0.10))
		...
		&"mage_fire_burst":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.44, -0.10, -0.34), Vector3(0.30, 0, 0.34), Vector3(0.36, -0.06, 0.10))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.58, 0, -0.12), Vector3(0.40, 0, 0.14), Vector3(0.50, 0, 0.04))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.40, -0.06, 0.40), Vector3(0.54, 0, 0.46), Vector3(0.44, -0.04, 0.34))
			_set_phases(data, LEFT_HIP, Vector3(0.10, 0, 0), Vector3(0.18, 0, 0), Vector3(0.08, 0, 0))
		&"frost_staff_shard":
			_set_phases(data, TORSO, Vector3(-0.10, 0.18, 0), Vector3(-0.16, -0.12, 0), Vector3(-0.05, -0.04, 0))
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.52, -0.08, -0.32), Vector3(0.36, 0, 0.18), Vector3(0.44, -0.04, -0.12))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.52, -0.08, 0.32), Vector3(0.36, 0, -0.18), Vector3(0.44, -0.04, 0.12))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.66, 0, -0.18), Vector3(0.48, 0, 0.12), Vector3(0.56, 0, -0.08))
			_set_phases(data, LEFT_ELBOW, Vector3(0.66, 0, 0.18), Vector3(0.48, 0, -0.12), Vector3(0.56, 0, 0.08))
		...
		&"cleric_lightning_bolt":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.46, -0.10, -0.28), Vector3(0.28, 0, 0.30), Vector3(0.36, -0.05, 0.08))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.42, -0.06, 0.38), Vector3(0.52, 0, 0.44), Vector3(0.42, -0.04, 0.34))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.58, 0, -0.14), Vector3(0.38, 0, 0.14), Vector3(0.48, 0, 0.02))
		...
		&"cleric_healing_blessing":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.48, -0.08, -0.46), Vector3(0.38, 0, 0.22), Vector3(0.44, -0.04, -0.28))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.48, -0.08, 0.46), Vector3(0.38, 0, -0.22), Vector3(0.44, -0.04, 0.28))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.60, 0, -0.16), Vector3(0.46, 0, 0.10), Vector3(0.54, 0, -0.08))
			_set_phases(data, LEFT_ELBOW, Vector3(0.60, 0, 0.16), Vector3(0.46, 0, -0.10), Vector3(0.54, 0, 0.08))
			_set_phases(data, LEFT_HIP, Vector3(0.08, 0, 0), Vector3(0.16, 0, 0), Vector3(0.06, 0, 0))
		...
		&"warlock_chaos_bolt":
			...
			_set_phases(data, RIGHT_SHOULDER, Vector3(0.56, -0.12, -0.38), Vector3(0.32, 0, 0.38), Vector3(0.42, -0.06, 0.14))
			_set_phases(data, LEFT_SHOULDER, Vector3(0.46, -0.06, 0.36), Vector3(0.58, 0, 0.46), Vector3(0.48, -0.04, 0.30))
			_set_phases(data, RIGHT_ELBOW, Vector3(0.68, 0, -0.22), Vector3(0.42, 0, 0.18), Vector3(0.56, 0, -0.10))
