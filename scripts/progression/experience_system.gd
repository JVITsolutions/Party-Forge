...
var experience_multiplier: float:
	get:
		return float(_configured_multiplier_percent) / 100.0
...
func configure_context(context: PlayerRunContext, member_id: int) -> void:
	...
	run_context = context
	leader_member_id = member_id if context != null and member_id > 0 else 0
	if run_context != null and not run_context.member_level_ready.is_connected(callback):
