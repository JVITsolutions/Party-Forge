...
`git archive HEAD` created the cold project only after the runner/test commit existed. Before import, the extracted tree contained neither `.git`, `.godot`, nor `.worktrees`; the two acceptance runners were present. Untracked sidecars from the development worktree were therefore excluded.
...
| 3 | `--headless --path . --quit-after 240 --script res://tests/integration/ground_item_pickup_input_runner.gd` | 0 | 1.889 s | Mouse, controller, full-inventory, foreign-owner, and overall integration PASS markers; 1 integration runner | Headless/default plus automated viewport-resize fixture | Input events are automated simulation, not physical-controller acceptance. | `C:\Users\Jacob\AppData\Local\Temp\party-forge-live-loot-task13-7b70000e-evidence\step2-03-ground-item-pickup-input.log` |
| 4 | `--headless --path . --quit-after 300 --script res://tests/integration/equipment_ledger_responsive_runner.gd` | 0 | 2.902 s | Exactly three `TASK10_EQUIPMENT_LEDGER_RESOLUTION_PASS` markers; `TASK10_EQUIPMENT_LEDGER_MEMBER_24_PASS`; responsive summary PASS; 1 integration runner | 1920x1080, 2560x1440, 3840x2160 | Automated geometry, focus, tooltip-layer, and equipment-refresh checks; no screenshots or visual judgment. | `C:\Users\Jacob\AppData\Local\Temp\party-forge-live-loot-task13-7b70000e-evidence\step2-04-equipment-ledger-responsive.log` |
...
Archive preparation exited 0 in 36.5 s and recorded `ARCHIVE_HEAD=fa2476980f5e674f212fe7fa649c3d036e625d15`; its path and exclusion checks are in `C:\Users\Jacob\AppData\Local\Temp\party-forge-live-loot-task13-7b70000e-evidence\cold-paths.txt`.
