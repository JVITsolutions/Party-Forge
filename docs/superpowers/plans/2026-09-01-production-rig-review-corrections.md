...
**Interfaces:**
- Consumes: four implementation commits plus three post-implementation plan-only provenance corrections after `correctionImplementationBase`.
...
Require exactly seven first-parent commits after `correctionImplementationBase`, in order:
...
Require the first four commits' union to equal the seven File Responsibility Map paths. Require the fifth, sixth, and seventh commits to modify only `docs/superpowers/plans/2026-09-01-production-rig-review-corrections.md`, with the fifth parent equal to the Task C commit, the sixth parent equal to the fifth commit, the seventh parent equal to the sixth commit, and all three exact subjects above. Require the complete seven-commit changed-path union to equal the seven product paths plus that one plan path, no merge commits, `git diff correctionImplementationBase..HEAD --check` exit `0`, clean tracked/index state, fixture byte identity, and absent sentinels.
...
- branch, worktree, unchanged `correctionImplementationBase`, four implementation hashes/parents/subjects, all three post-implementation plan-only correction hashes/parents/subjects, the exact seven-path implementation union, and the complete eight-path changed union;
