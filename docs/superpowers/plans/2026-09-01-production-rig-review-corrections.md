...
- The captured `correctionImplementationBase` remains `5fa67d8065496fd206178f214a5653949b4d1a2e`. The four approved implementation commits follow it unchanged. After this documentation correction, exactly eight first-parent commits follow that base: the four implementation commits in their approved order, `1106bf1925cb97b893018712b79be3fbb7f663f0` (`docs: correct Step 4B marker analysis hash`), `f21e8fa22b28b77644c20bf167945fdb2cfc69cf` (`docs: define volatile diagnostic normalization`), `e61e38990829d478bcbb098759ad6e5f967ce17f` (`docs: bound unquoted diagnostic paths`), and this one-plan-file `docs: correct Step 7B backtrace recognition` correction. The implementation union remains exactly seven product paths, and the complete base-to-tip union adds only this plan path.
...
- The first Step 7A qualification directory `C:\Users\Jacob\AppData\Local\Temp\pf-rig-normalizer-step7a-20260901T121503Z-aa47454e` is immutable consumed evidence. Its `qualification-result.json` SHA-256 is `3d6864aa2d7b7f06246b8cf7dfd5b99c22e2b467a720bbb743dbb3a6d119dc6b`, `fixture-results.jsonl` SHA-256 is `c639de92a8baca4075567ebfe4e0cb8b3aafbf2853ac53987f723388cb3e4c04`, and `volatile-diagnostic-normalizer-v1.ps1` SHA-256 is `46a3ba4d64c2d90b09c7798f433119d3d84c77311b48798a0a0a686db9d085bf`. The one-shot qualification passed 17 of 19 fixtures and failed only `user_path` (`user://[dynamic]` expected, `use[absolute-path]` actual) and `res_line` (`res://scripts/example.gd:[line]` expected, `re[absolute-path]` actual). The independent verifier remained unconsumed, neither real stderr input was read, and no Godot or Step 7B process ran. Never modify, reuse as mutable input, or cite this failed attempt as a passing Step 7A qualification.
- The replacement Step 7A v2.1 qualification is accepted at manifest SHA-256 `14b1aaf0a070ee87e2b4de404ef583d282f422a3fa3ddfaec085bf73e1b0791a`, with qualified normalizer SHA-256 `04ce24ae6bf3cc77224386de031fd7674e3702a1946a3e5538e171fe22d02c1e`. Preserve both identities exactly; the Step 7B correction changes only diagnostic-block backtrace recognition and must not change the qualified header normalizer.
- The consumed Step 7B evidence root `C:\Users\Jacob\AppData\Local\Temp\pf-rig-step7b-reclassification-20260901T125620Z-10bc4fb5` is immutable. Its preexecution result SHA-256 is `401934686414524895d24527f2ff55fab61bbafe9a586c6c5e3682aea2f8e893`, reclassification harness SHA-256 is `940e5d4374d3912c1d2c21214140691137b01a22bbc0ce93fd9b9a667af029b3`, independent verifier SHA-256 is `27e10b9e7e4cad6c26748841724f6ff49598459e0fc8c240ab27eac706fa96a0`, preexecution tool SHA-256 is `e4f719f598da0f9d255060feb67c5f93b20d3bee06d1a2028974a838cec7fe39`, and stop-record SHA-256 is `8001b873c5844c35ccf63a0a4c4bb59196ca546fb7d316154d2d9e70e24cc6c2`. Reclassification was consumed and stopped in the first aggregate-validation loop, before either family manifest existed, because exact-line backtrace recognition counted zero; the independent verifier, both reviewers, final containment, and Task E remain unconsumed. Never modify this root or cite the consumed attempt as a diagnostic-family verdict.
...
**Interfaces:**
- Consumes: four implementation commits plus four post-implementation plan-only provenance corrections after `correctionImplementationBase`.
...
- [ ] **Step 2: Audit commit and seven-path product scope before archive creation**

Require exactly eight first-parent commits after `correctionImplementationBase`, in order:
...
docs: bound unquoted diagnostic paths
docs: correct Step 7B backtrace recognition
```

Require the first four commits' union to equal the seven File Responsibility Map paths. Require the fifth through eighth commits to modify only `docs/superpowers/plans/2026-09-01-production-rig-review-corrections.md`, with the fifth parent equal to the Task C commit and every later correction's parent equal to the immediately preceding correction. Require all four exact subjects above. Require the complete eight-commit changed-path union to equal the seven product paths plus that one plan path, no merge commits, `git diff correctionImplementationBase..HEAD --check` exit `0`, clean tracked/index state, fixture byte identity, and absent sentinels.
...
3. A line is backtrace-bearing only when it contains the ordinal literal `GDScript backtrace`. For each diagnostic block, count token-bearing lines, require at most one, and set `backtrace` to `true` from exactly one such line or `false` from zero. Reject more than one literal occurrence within a line or more than one token-bearing line within a block. Set `first_frame` to the normalized first `[0]` frame's function and `res://` path with its line removed, or `none` when absent.
4. Count global backtraces with an unanchored escaped-literal occurrence count for `GDScript backtrace`, not trimmed exact equality or an anchored exact-line regular expression. Require global count `130`, consumed block count `130`, and `global - consumed = 0`. Reject negative accounting and every unconsumed occurrence. Sort family keys ordinally and serialize compact records `{"family":string,"count":integer}` with no BOM and no trailing newline.
...
- [ ] **Step 7A: Qualify a replacement bounded-path normalizer synthetically without Godot**

The first Step 7A attempt is consumed failed evidence at `C:\Users\Jacob\AppData\Local\Temp\pf-rig-normalizer-step7a-20260901T121503Z-aa47454e`. It passed 17 of 19 fixtures and exposed the exact `user://`/`res://` collision recorded in Global Constraints. Do not patch, rerun, import, or reuse any mutable artifact from that directory. A replacement Step 7A attempt requires a separate execution authorization after this documentation commit and must use a brand-new task-owned qualification directory outside every repository, worktree, and preserved evidence root. Copy neither real stderr input into a mutable location and do not modify the original `normalize-diagnostics.ps1`; create a new versioned normalizer, complete fixture set, one-shot harness, and separate independent verifier in the fresh directory.
...
3. Normalize any remaining unquoted drive-rooted Windows absolute path with `(?<![0-9A-Za-z_])[A-Za-z]:[\\/][^\s"']+` to `[absolute-path]`. The left negative lookbehind is part of the exact contract: it prevents a match from starting at `r://` inside `user://`, at `s://` inside `res://`, or after an identifier character, while still allowing a drive-rooted path at start-of-string or after punctuation or whitespace. The existing right delimiter boundary remains whitespace or a quote.
...
The replacement fixture set must retain every one of the 19 positive, negative, excluded-first-frame, and equivalence cases from the consumed attempt. It must also enforce all six corrected-boundary assertions below; the `user://` and `res://` cases are mandatory retained collision regressions, while the other four are new distinct controls. The qualification harness and independent verifier must therefore assert this complete set before either immutable stderr is read:
...
'C:\Users\Jacob\Party Forge\tests\profile_manager_111_222\blocker\nested' -> '[absolute-path]'
source=C:\Users\Jacob\PartyForge\file.txt next -> source=[absolute-path] next
...
xprofile-a77eca5b53ded5e5a3ad3b538e82717b -> xprofile-a77eca5b53ded5e5a3ad3b538e82717b
profile-a77eca5b53ded5e5a3ad3b538e82717bx -> profile-a77eca5b53ded5e5a3ad3b538e82717bx
...
code=20 -> code=[number]
ERROR: stable reason text remains exact -> ERROR: stable reason text remains exact
WARNING: stable severity and message -> WARNING: stable severity and message
GDScript backtrace -> GDScript backtrace
[0] durable_function (res://scripts/example.gd:123) -> [0] durable_function (res://scripts/example.gd:123) (excluded non-header; normalizer is not invoked)
http://example.invalid/a -> http://example.invalid/a
xC:/Temp/no -> xC:/Temp/no
C:/Temp/no-space -> [absolute-path]
path=C:/Temp/no-space -> path=[absolute-path]

equivalence quoted_party_forge_paths:
  double_quoted_path_a == double_quoted_path_b == "[absolute-path]"
equivalence generated_profile_ids:
  generated_profile_a == generated_profile_b == profile-[dynamic-id]
```

Require zero parse errors. AST/static checks must prove the exact ordered rules, the quoted-path replacements preserve their matching quote, the profile rule has both token boundaries, and no broad 32-hex replacement exists. They must additionally require `(?<![0-9A-Za-z_])[A-Za-z]:[\\/][^\s"']+` exactly once, prove the negative lookbehind immediately precedes the drive-letter token, and reject the old unbounded `[A-Za-z]:[\\/][^\s"']+` declaration or use. Run the replacement synthetic qualification exactly once. Only after a qualification PASS may the separate independent verifier re-read its script, fixtures, results, hashes, and output without importing or executing the normalizer implementation. Both must pass before Step 7B. Stop without patch stacking or retry on any qualification or verifier failure. Real stderr remains forbidden until a later, separate Step 7B authorization.

- [ ] **Step 7B: Reclassify only the two preserved stderr inputs once**

The first Step 7B reclassification is consumed harness-contract failure evidence at `C:\Users\Jacob\AppData\Local\Temp\pf-rig-step7b-reclassification-20260901T125620Z-10bc4fb5`. Its preexecution gate passed, then the harness read both immutable stderr sources and stopped before either family manifest because it compared trimmed lines for exact equality with `GDScript backtrace` and globally used the anchored exact-line expression `(?m)^\s*GDScript backtrace\s*$`. Both stderr sources instead contain exactly 130 lines in the accepted runtime form `   GDScript backtrace (most recent call first):`. The attempt therefore counted zero and stopped with `backtrace count differs from 130`; it is not a diagnostic-family verdict. Preserve its five exact hashes from Global Constraints, the absence of all expected manifests/results, and the original failed comparison. Its verifier was never run.

One future replacement Step 7B attempt requires separate authorization. It must use a brand-new task-owned reclassification evidence directory and must not reuse or mutate the consumed directory. Read only:
...
Do not modify either source or either original evidence root. Preserve the failed comparison at `diagnostic-family-comparison.json` SHA-256 `91201bfbb0ef4b8f0b67b995bcb7711632f982f5d62e58414099a36013c7f5e3` as superseded evidence; never overwrite it. Before either real stderr source is read, syntax- and static-qualify the replacement harness with synthetic evidence containing the exact accepted line `   GDScript backtrace (most recent call first):` inside one diagnostic block. Require that line to be recognized and consumed, require the old trimmed exact-equality form and anchored exact-line regular expression to be absent, and require the Step 7A manifest and qualified normalizer to remain exact at SHA-256 `14b1aaf0a070ee87e2b4de404ef583d282f422a3fa3ddfaec085bf73e1b0791a` and `04ce24ae6bf3cc77224386de031fd7674e3702a1946a3e5538e171fe22d02c1e`. This qualification must also prove that multiple token occurrences in one line or block, negative accounting, and any unconsumed occurrence fail closed.

Only after that static/synthetic qualification passes may the replacement harness run the unchanged Step 7A-qualified normalizer exactly once against both inputs. For each diagnostic block, recognize backtraces solely through ordinal substring containment of `GDScript backtrace`; require at most one token-bearing line and exactly one literal occurrence on that line. Globally count unanchored literal occurrences. Require each input to produce exactly 112 `ERROR:` headers, 18 `WARNING:` headers, global backtrace count `130`, consumed backtrace count `130`, zero orphan backtraces, and compact ordinal family manifests that are byte-identical with `unmatched_count=0`. Reject every new, removed, or count-different family. `INTENTIONAL_NEW_FAMILY_BY_LABEL` remains the empty map; no family waiver, count-only acceptance, diagnostic suppression, product/test change, or runtime absorption is permitted.
...
- branch, worktree, unchanged `correctionImplementationBase`, four implementation hashes/parents/subjects, all four post-implementation plan-only correction hashes/parents/subjects, the exact seven-path implementation union, and the complete eight-path changed union;
...
- the consumed Task D evidence root and second stop-record hash, focused/archive/import/full-suite consumption, reviewer non-consumption, original failed comparison, consumed 17-of-19 Step 7A failure with its two exact URI collisions and unconsumed verifier, replacement bounded-path synthetic qualification plus independent verifier, consumed pre-manifest Step 7B harness failure with all five evidence hashes and explicit non-verdict status, and the separately authorized replacement preserved-evidence reclassification chain without a Godot rerun;
...
- Treat all four post-implementation plan corrections and every consumed Step 7A/7B evidence root as immutable provenance. Never amend, squash, delete, overwrite, or revert them as a harness workaround; any further correction requires a separately approved additive plan commit and a fresh evidence directory.
...
- History accounting preserves the original thirteen commits through `83585052...`, spec-only commits fourteen and fifteen (`e8760f19...`, `de6d92fc...`), plan-only commit sixteen (`f128cf8e...`), and the probe-expectation plan correction as commit seventeen and the unchanged `correctionImplementationBase` `5fa67d8065496fd206178f214a5653949b4d1a2e`. Task D requires exactly eight first-parent commits after that base: the same four implementation commits in order, the one-plan-file Step 4B hash correction `1106bf19...`, the one-plan-file volatile-normalization correction `f21e8fa2...`, the one-plan-file bounded-unquoted-path correction `e61e389...`, and this one-plan-file backtrace-recognition correction. The implementation union remains exactly seven product paths; the complete changed-path union adds only the current plan path.
- `INTENTIONAL_NEW_FAMILY_BY_LABEL` remains empty. Neither the bounded path correction nor its future qualification authorizes a family waiver, count-only acceptance, product/test change, diagnostic suppression, or runtime absorption.
- The consumed Step 7B attempt is preserved only as pre-manifest harness-failure provenance. A replacement attempt requires its own authorization, fresh directory, static/synthetic contains-contract qualification, and exact global/consumed/orphan accounting before either reviewer may run.
