...
4. Accept incoming damage through `receive_damage(amount)` rather than changing health from unrelated systems.
5. Return a combat-target record that becomes unavailable after death.
6. Emit exactly one `reward_dropped(experience, position)` signal on defeat.
7. Stop moving, complete defeat once, and `queue_free()` itself.
...
> **Party Forge convention:** All hostile actors join `hostile_actors`, use team ID `2`, expose the shared combat-target interface, and route terminal health through `EnemyActor.defeat()`.
...
| --- | --- | --- |
| `scenes/enemies/swarmer.tscn` | `scripts/enemies/swarmer.gd` | Chases the nearest living party actor and deals contact damage. |
...
2. Right-click the folder, choose **Create New > Resource**, select `EnemyDefinition`, and save it as `training_brute.tres`.
3. Enter every value from the specification. Select `SWARMER` from the Behavior list instead of typing an enum integer.
4. Save, close, and reopen the Resource. Confirm its Inspector path and values.
...
- Contact reduces party health by `12.0` per successful hit, subject to the Swarmer cooldown.
- Its health starts at `40.0`, damage reaches `receive_damage()`, and death happens once.
...
2. Move its definition to a permanent data path, then add that exact `res://` path to `GameCatalog.ENEMY_PATHS` in `scripts/data/game_catalog.gd`.
...
3. Implement movement and attack decisions in a focused method called from `_physics_process(delta)`.
...
4. Test `receive_damage()`, terminal health, one reward, correct experience value, and one cleanup.
...
6. Delete the enemy scene, definition, behavior script if unique, and only their generated UID files.
