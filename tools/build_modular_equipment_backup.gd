...
class LocalFilesystem extends RefCounted:
	var _output_root := ""


	func probe_path(path: String, require_exists: bool) -> Dictionary:
		var normalized := _normalize(path)
		var exists := FileAccess.file_exists(normalized) or DirAccess.dir_exists_absolute(normalized)
		if require_exists and not exists:
			return {"error": "path does not exist: %s" % normalized, "path": ""}
		return {"error": "", "path": normalized, "exists": exists}
	...
	func directory_state(path: String) -> Dictionary:
		...
		return {
			"error": "",
			"exists": true,
			"empty": DirAccess.get_files_at(normalized).is_empty() and DirAccess.get_directories_at(normalized).is_empty(),
		}


	func configure_output_root(path: String) -> Dictionary:
		_output_root = _normalize(path)
		return {"error": ""}


	func ensure_directory(path: String) -> Dictionary:
		if _output_root.is_empty():
			return {"error": "output root was not configured", "created_paths": []}
		var normalized := _normalize(path)
		if not _is_same_or_descendant(normalized, _output_root):
			return {"error": "directory is outside configured output root", "created_paths": []}
		...
		for directory: String in missing:
			...
			if error != OK:
				return {"error": "directory create failed code=%d path=%s" % [error, directory], "created_paths": created}
			created.append(directory)
		return {"error": "", "created_paths": created}


	func read_file(path: String) -> Dictionary:
		var file := FileAccess.open(_normalize(path), FileAccess.READ)
		if file == null:
	...
	func copy_file_verified(source_path: String, destination_path: String) -> Dictionary:
		...
		return {
			"error": String(destination.get("error", "")),
			"created": true,
			"source": source,
			"destination": destination,
			"equal": bytes == (destination.get("bytes", PackedByteArray()) as PackedByteArray),
		}


	func create_file_exclusive(path: String, bytes: PackedByteArray) -> Dictionary:
		var normalized := _normalize(path)
		if _output_root.is_empty() or not _is_same_or_descendant(normalized, _output_root):
			return {"error": "file is outside configured output root", "created": false}
	...
	func publish_no_replace(pending_path: String, final_path: String) -> Dictionary:
		var pending := _normalize(pending_path)
		var final := _normalize(final_path)
		if _output_root.is_empty() or not _is_same_or_descendant(pending, _output_root) or not _is_same_or_descendant(final, _output_root):
			return {"error": "publication is outside configured output root", "published": false}
		if not FileAccess.file_exists(pending):
			return {"error": "pending manifest is missing", "published": false}
		if FileAccess.file_exists(final) or DirAccess.dir_exists_absolute(final):
			return {"error": "collision", "published": false}
		var error := DirAccess.rename_absolute(pending, final)
		if error != OK:
...
class BackupService extends RefCounted:
	func build_backup_with_filesystem(request: Dictionary, inventory_paths: PackedStringArray, source_metadata: Dictionary, filesystem: RefCounted) -> Dictionary:
		if filesystem == null:
			return _failed_result("%s field=filesystem reason=local adapter required" % ERROR_PREFIX)
		if not filesystem.has_method(&"copy_file_verified"):
			return _failed_result("%s field=filesystem reason=verified copy operation is required" % ERROR_PREFIX)
		...
		if not filesystem.has_method(&"configure_output_root"):
			return _failed_result("%s field=filesystem reason=output root configuration is required" % ERROR_PREFIX)
	...
	func _validate_request(request: Dictionary, inventory_paths: PackedStringArray, source_metadata: Dictionary, filesystem: RefCounted) -> String:
		var local_path_error := _validate_local_request_paths(request)
		if not local_path_error.is_empty():
			return local_path_error
	...
	func _validate_local_request_paths(request: Dictionary) -> String:
		for field: String in ["source_root", "output"]:
			var raw := String(request.get(field, ""))
			if _is_nonlocal_path_form(raw):
				return "%s field=%s reason=must be a local absolute path" % [ERROR_PREFIX, field]
			if raw.is_empty() or not raw.is_absolute_path():
				return "%s field=%s reason=must be absolute" % [ERROR_PREFIX, field]
			if not _is_local_absolute_path(raw):
				return "%s field=%s reason=must be a local absolute path" % [ERROR_PREFIX, field]
		return ""
	...
	func _validate_metadata(request: Dictionary, source_metadata: Dictionary) -> String:
		...
		var source_root := _normalized_absolute(String(request.get("source_root", "")))
		var git_toplevel := _normalized_absolute(String(source_metadata.get("toplevel", "")))
		if source_root.to_lower() != git_toplevel.to_lower():
			return "%s field=source_root reason=actual Git top-level must equal source root" % ERROR_PREFIX
		return ""
	...
	func _is_nonlocal_path_form(path: String) -> bool:
		var slashes := path.replace("\\", "/")
		return slashes.begins_with("//")


	func _is_local_absolute_path(path: String) -> bool:
		var slashes := path.replace("\\", "/")
		if slashes.length() < 3 or slashes[1] != ":" or slashes[2] != "/":
			return false
		return slashes[0].to_lower() in "abcdefghijklmnopqrstuvwxyz"
...
func _initialize() -> void:
	...
	var local_path_error := validate_local_request_paths(request)
	if not local_path_error.is_empty():
		_fail(local_path_error)
		return
	var source_root := String(request.get("source_root", ""))
	var filesystem := LocalFilesystem.new()
...
func validate_local_request_paths(request: Dictionary) -> String:
	return BackupService.new()._validate_local_request_paths(request)
...
func new_local_filesystem() -> RefCounted:
	return LocalFilesystem.new()
