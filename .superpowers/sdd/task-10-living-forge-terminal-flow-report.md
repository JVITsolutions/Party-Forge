...
## Residual replay-truth and canonical-selection repair

This repair started from committed head `b011084d8723010080f24d89d60dc82cc9228eb5`. While its disjoint tests were running, the separately owned Armoury help-copy change advanced the branch to `20afa01d49058f9db4f157c6f81d23763764d1fb`; that commit was preserved without modification. The binding schema-6 adjudication remains unchanged: `terminal_recovery_overflow` is still the explicit canonical empty container with its stable ID, kind, owner, capacity, and empty slots.

- Current-live terminal duplicate truth used `--script res://tests/focused_test_runner.gd -- tests/unit/test_run_terminal_flow.gd`. The fixture committed a normal terminal resolution, drifted only the current live `terminal_resolution.snapshot.elapsed_seconds`, proved the journal `result_profile` remained byte-for-byte intact, and retried the exact original source/request. Before production edits, RED was `TERMINAL_LIVE_REPLAY_RED_EXIT:1`, `TEST_SUMMARY: FAIL (1 failures)`, with the exact failure `ordinary terminal duplicate rejects drift in the current live resolved record: expected true`. GREEN was `TERMINAL_LIVE_REPLAY_GREEN_EXIT:0`, `TEST_SUMMARY: PASS (0 failures)`. `resolve_terminal_source()` now loads the current profile through the same `ProfileMutationService` store, validates that profile, requires its live resolved record/applied transaction to pass `RunRecoveryService.verify_terminal_safety()`, matches the live record to the supplied source/request, and returns that validated live profile on an ordinary duplicate.
- Resolved selected-ID canonicality used `--script res://tests/focused_test_runner.gd -- tests/unit/test_run_terminal_recovery_safety.gd`. The structurally valid fixture coordinated the same noncanonical permutation across the resolved record and accepted extraction while preserving a canonical applied request/fingerprint/receipt and exact result snapshot. Before production edits, RED was `NONCANONICAL_SELECTION_RED_EXIT:1`, `TEST_SUMMARY: FAIL (2 failures)`: completion incorrectly succeeded and its resulting write violated the write-free assertion. GREEN was `NONCANONICAL_SELECTION_GREEN_EXIT:0`, `TEST_SUMMARY: PASS (0 failures)`, retaining only the established injected `JSON_STORE_CLEANUP_DEBT` warnings. Resolved safety now requires reconstructed canonical selected IDs to equal `record.selected_item_ids` exactly before request/fingerprint/receipt acceptance.
- Combined flow/recovery/resolution verification used the focused runner with `tests/unit/test_run_terminal_flow.gd tests/unit/test_run_terminal_recovery_safety.gd tests/unit/test_run_resolution_service.gd`. It ended `REPLAY_CANONICAL_COMBINED_GREEN_EXIT:0`, `TEST_SUMMARY: PASS (0 failures)`, with only the established injected cleanup-debt warnings.
- Final integrity verification printed `REPLAY_CANONICAL_DIFF_CHECK_EXIT:0`; the only changed paths were the three production authorities, two focused test files, and this report.
- Historical `.superpowers/sdd/task-10-report.md` remained blob `95a538c9668fa8e6381166dc8c488b502956cd31`. No schema/codec/migration representation was changed.
