...
- The Task 13 defeat validator now iterates the authoritative current choice count, bounds-checks retained card storage, requires visible enabled typed cards, uses visible empty-offer recovery, and emits recovery when no current choice is selectable. A real `LevelUpPanel` fixture first presents five enabled choices to allocate the retained card pool, then presents one disabled current choice and explicitly proves the four extra retained cards are hidden before the actual driver runs. The same fixture proves an empty offer activates recovery without an out-of-range access or stall.
