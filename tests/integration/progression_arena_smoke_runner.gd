...
func _run() -> void:
	...
	_assert(setup_manager.refresh_profile(profile_id).is_empty(), "profile observation refreshes after authoritative run checkout")
	...
	_assert(before_profile != null, "selected profile is available before run progression")
	_assert(not before_bytes.is_empty(), "profile bytes exist before run progression")
	...
	var xp_bar := main.get_node_or_null("HUD/Margin/CombatStatus/Experience") as ProgressBar
	_assert(xp_bar != null, "HUD exposes the signal-driven Living Forge XP path")
	if xp_bar != null:
