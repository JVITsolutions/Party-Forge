...
func _exercise_recruitment(class_id: StringName, display_name: String) -> void:
	...
	_assert(party.members.size() == 2 and party.members[1].class_definition.id == class_id, "%s recruitment commits the exact class" % display_name)
	var context := main.get("active_run_context") as PlayerRunContext
	var actor := context.actor_for(party.members[1].member_id) if context != null else null
