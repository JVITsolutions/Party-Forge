...
func _run() -> void:
	...
	var victory := ProfileTestSupport.commit_city_victory(profile_id, "input-runner-first-victory", _profile_root)
	_assert(victory.ok(), "input profile commits first-victory City discovery")
	_assert(victory.profile.prologue_state == ProfileState.PrologueState.NOT_STARTED, "input first victory leaves prologue state unchanged")
	...
	await _frames(2)
	_assert(city_button.visible and not city_button.disabled, "first-victory discovery exposes the production City route")
	...
	_assert(canvas.selected_node_id() == &"equipment-registry", "device-0 left stick navigates down the radial Forge route")
	...
	var future_path := ProfileStore.new().profile_path(profile_id, _profile_root)
	var future_bytes := FileAccess.get_file_as_bytes(future_path)
	var future_points := manager.active_profile().passive_points_available
	await _key(viewport, KEY_ENTER)
	_assert(not (screen.get_node("Overlay/Confirmation") as Control).visible, "Player Mode future-node input opens no confirmation")
	_assert("shared-lessons-1" not in manager.active_profile().tree_allocations.get(TREE_ID, []), "Player Mode future-node input persists no allocation")
	_assert(manager.active_profile().passive_points_available == future_points and FileAccess.get_file_as_bytes(future_path) == future_bytes, "Player Mode future-node rejection spends no point and changes no profile bytes")
	_assert((screen.get_node("Overlay/Frame/Layout/Body/DetailScroll/DetailBody/DetailSections") as Label).text.contains("Coming Soon"), "future node presents Coming Soon readiness copy")

	_assert(canvas.select_node(&"logistics-district-charter"), "Player Mode can inspect the visible Logistics charter")
	var charter_bytes := FileAccess.get_file_as_bytes(future_path)
	await _key(viewport, KEY_ENTER)
	_assert(not (screen.get_node("Overlay/Confirmation") as Control).visible, "missing-target charter input opens no confirmation")
	_assert("logistics-district-charter" not in manager.active_profile().tree_allocations.get(TREE_ID, []), "missing-target charter persists no allocation")
	_assert(FileAccess.get_file_as_bytes(future_path) == charter_bytes, "missing-target charter rejection changes no profile bytes")
	_assert((screen.get_node("Overlay/Frame/Layout/Body/DetailScroll/DetailBody/DetailSections") as Label).text.contains("District tree not installed"), "missing-target charter presents its exact readiness copy")
	...
	_assert(canvas.select_node(&"open-market"), "Developer preview can inspect a future connected node")
	var developer_bytes := FileAccess.get_file_as_bytes(future_path)
	var developer_profile_before := manager.active_profile().to_dictionary()
	await _joy_button(viewport, JOY_BUTTON_A)
	_assert(not (screen.get_node("Overlay/Confirmation") as Control).visible, "Developer preview future-node input opens no confirmation")
	_assert(manager.active_profile().to_dictionary() == developer_profile_before and FileAccess.get_file_as_bytes(future_path) == developer_bytes, "Developer preview cannot persist a Player Mode-invalid future allocation")
	_assert((screen.get_node("Overlay/Frame/Layout/Body/DetailScroll/DetailBody/DetailSections") as Label).text.contains("Coming Soon"), "Developer preview retains future readiness copy")
	_assert(screen.is_open() and not settings.is_open(), "rejected Developer preview input keeps the passive-tree child open")
	_assert(canvas.select_node(&"logistics-district-charter"), "Developer preview can inspect the visible Logistics charter")
	var developer_charter_bytes := FileAccess.get_file_as_bytes(future_path)
	var developer_charter_before := manager.active_profile().to_dictionary()
	await _joy_button(viewport, JOY_BUTTON_A)
	_assert(not (screen.get_node("Overlay/Confirmation") as Control).visible, "Developer preview missing-target charter opens no confirmation")
	_assert("logistics-district-charter" not in manager.active_profile().tree_allocations.get(TREE_ID, []), "Developer preview missing-target charter persists no allocation")
	_assert(manager.active_profile().to_dictionary() == developer_charter_before and FileAccess.get_file_as_bytes(future_path) == developer_charter_bytes, "Developer preview missing-target charter changes no profile state or bytes")
	_assert((screen.get_node("Overlay/Frame/Layout/Body/DetailScroll/DetailBody/DetailSections") as Label).text.contains("District tree not installed"), "Developer preview missing-target charter retains exact readiness copy")
	await _key(viewport, KEY_ESCAPE)
