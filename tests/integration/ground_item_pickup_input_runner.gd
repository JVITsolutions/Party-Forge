...
func _test_real_controller_flow() -> void:
	...
	registry.add(_record(&"p1-hidden", &"player_1", Vector3(0.5, 0.0, 0.0), 1))
	registry.add(_record(&"p1-far", &"player_1", Vector3(30.0, 0.0, 0.0), 2))
	...
	controller.call(&"configure", registry, {}, func(record: GroundItemRecord) -> Dictionary: return _detail(record), null, chests, tooltip_layer)
	var controller_owned_index := controller.get("_spatial_index") as RefCounted
	var replacement_registry := GroundItemRegistry.new()
	controller.call(&"configure", replacement_registry, {}, func(record: GroundItemRecord) -> Dictionary: return _detail(record), null, chests, tooltip_layer)
	...
	var modal := [false]
	controller.call(&"configure_interaction", index, targeting, pickup, contexts, 5.0, func(record: GroundItemRecord) -> bool: return record.drop_id != &"p1-hidden", func() -> bool: return modal[0])
	...
	_assert(controller.call(&"selection_for_owner", &"player_1") == &"p1-near", "device 0 excludes hidden and far P1 entries")
	modal[0] = true
	...
	var chest := (controller.get("_chest_by_drop") as Dictionary)[&"p1-near"] as Node3D
	var anchor := chest.call(&"tooltip_anchor") as Control
	anchor.set_meta("input_owner", "player_2")
	anchor.pressed.emit()
	_assert(forwarded.is_empty(), "foreign mouse pointer owner is rejected")
	anchor.set_meta("input_owner", "player_1")
	anchor.pressed.emit()
