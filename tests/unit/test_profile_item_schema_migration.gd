...
func run() -> Array[String]:
	...
	_test_schema_five_adds_empty_terminal_documents(failures)
...
func _test_schema_five_adds_empty_terminal_documents(failures: Array[String]) -> void:
	var legacy := ProfileState.new_profile("profile-migrate-terminal", "Terminal", 1000).to_dictionary()
	legacy["schema_version"] = 5
	legacy.erase("terminal_resolution")
	legacy.erase("terminal_recovery_overflow")
	var migrated := ProfileMigrator.migrate_document(legacy)
	TestAssertions.truthy(migrated.ok(), "schema five terminal migration succeeds", failures)
	if migrated.ok():
		TestAssertions.equal(migrated.profile.get("terminal_resolution"), {}, "schema five migration invents no terminal receipt", failures)
		var overflow: Variant = migrated.profile.get("terminal_recovery_overflow")
		TestAssertions.equal((overflow as Dictionary).get("slots") if overflow is Dictionary else null, {}, "schema five migration invents no overflow contents", failures)
...
func _test_complete_schema_one_migrates_recursively(failures: Array[String]) -> void:
	...
	TestAssertions.equal(migrated_profile.schema_version if migrated_profile != null else -1, 6, "profile migrates one version at a time through schema six", failures)
	...
	TestAssertions.equal(migrated_profile.resumable_run if migrated_profile != null else {"missing": true}, {}, "schema-one opaque recovery marker is revoked", failures)
	...
	if migrated_profile != null:
		...
		TestAssertions.equal(nested["schema_version"], 6, "nested result snapshot migrates one version at a time through schema six", failures)
		...
		TestAssertions.equal(nested.get("resumable_run"), {}, "nested schema-one opaque recovery marker is revoked", failures)
...
func _test_json_parsed_schema_one_migrates_to_complete_canonical_document(failures: Array[String]) -> void:
	...
	if migrated_profile != null:
		...
		TestAssertions.equal(_sorted_strings(Array(actual.keys())), _sorted_strings(ProfileCodec.CURRENT_FIELDS), "migrated root has the exact current fields", failures)
		...
		TestAssertions.equal(_sorted_strings(Array(nested.keys())), _sorted_strings(ProfileCodec.CURRENT_FIELDS), "migrated result snapshot has the exact current fields", failures)
...
func _test_schema_two_items_and_progression_migrate_losslessly(failures: Array[String]) -> void:
	...
	original["resumable_run"] = {"legacy_marker": "schema-two-root"}
	(((original["applied_transactions"] as Dictionary)["storage-001"] as Dictionary)["result_profile"] as Dictionary)["resumable_run"] = {"legacy_marker": "schema-two-snapshot"}
	var before := original.duplicate(true)
	var before_text := JSON.stringify(original, "\t", false)
	TestAssertions.equal(ProfileCodec.validate_loadable_document(original), "", "schema-two source validation admits opaque recovery for revocation", failures)
	TestAssertions.truthy(ProfileCodec.validate_schema_two_document(original).contains("field=resumable_run"), "standalone schema-two validation requires revoked recovery", failures)
	var migrated: Variant = _migrate(original)
	...
	if profile != null:
		...
		TestAssertions.equal(actual["schema_version"], 6, "schema-two profile promotes through schema three to schema six", failures)
		...
		TestAssertions.equal(actual["resumable_run"], {}, "schema-two opaque recovery is revoked", failures)
		...
		TestAssertions.equal(actual_snapshot["resumable_run"], {}, "schema-two transaction snapshot opaque recovery is revoked", failures)
...
func _test_schema_three_adds_preferred_color_recursively(failures: Array[String]) -> void:
	...
	snapshot.erase("terminal_resolution")
	snapshot.erase("terminal_recovery_overflow")
	snapshot["updated_at_unix"] = 1001
	snapshot["resumable_run"] = {"legacy_marker": "schema-three-snapshot"}
	var legacy := snapshot.duplicate(true)
	legacy["updated_at_unix"] = 1002
	legacy["resumable_run"] = {"legacy_marker": "schema-three-root"}
	...
	TestAssertions.equal(ProfileCodec.validate_loadable_document(legacy), "", "schema-three source validation admits opaque recovery for revocation", failures)
	TestAssertions.truthy(ProfileCodec.validate_schema_three_document(legacy).contains("field=resumable_run"), "standalone schema-three validation requires revoked recovery", failures)
	...
	if migrated.profile != null:
		TestAssertions.equal(migrated.profile.resumable_run, {}, "schema-three opaque recovery is revoked", failures)
		var nested := ((migrated.profile.applied_transactions["legacy-color"] as Dictionary)["result_profile"] as Dictionary)
		TestAssertions.equal(nested.get("schema_version"), 6, "schema-three transaction snapshot advances to schema six", failures)
		...
		TestAssertions.equal(nested.get("resumable_run"), {}, "schema-three transaction snapshot opaque recovery is revoked", failures)
...
func _test_schema_four_recovery_migrates_recursively(failures: Array[String]) -> void:
	...
	if loaded.profile != null:
		...
		TestAssertions.equal(actual["schema_version"], 6, "schema-four recovery promotes to schema six", failures)
		...
		for field: String in ["item_state", "leader_member_id", "run_id", "run_player_id", "run_seed"]:
			var expected_value: Variant = persisted_recovery[field]
			var actual_value: Variant = (actual["resumable_run"] as Dictionary)[field]
			if field == "item_state":
				expected_value = ItemOwnershipState.decode(expected_value, GameCatalog.EQUIPMENT_CATALOG, GameCatalog.ITEM_FOUNDATION_CATALOG).state.to_dictionary()
				actual_value = ItemOwnershipState.decode(actual_value, GameCatalog.EQUIPMENT_CATALOG, GameCatalog.ITEM_FOUNDATION_CATALOG).state.to_dictionary()
			TestAssertions.equal(actual_value, expected_value, "migrated recovery preserves canonical legacy field %s" % field, failures)
		var nested := ((actual["applied_transactions"] as Dictionary)["legacy-recovery"] as Dictionary)["result_profile"] as Dictionary
		TestAssertions.equal(nested["schema_version"], 6, "schema-four transaction snapshot promotes recursively", failures)
		...
		for field: String in ["item_state", "leader_member_id", "run_id", "run_player_id", "run_seed"]:
			var expected_value: Variant = persisted_recovery[field]
			var actual_value: Variant = (nested["resumable_run"] as Dictionary)[field]
			if field == "item_state":
				expected_value = ItemOwnershipState.decode(expected_value, GameCatalog.EQUIPMENT_CATALOG, GameCatalog.ITEM_FOUNDATION_CATALOG).state.to_dictionary()
				actual_value = ItemOwnershipState.decode(actual_value, GameCatalog.EQUIPMENT_CATALOG, GameCatalog.ITEM_FOUNDATION_CATALOG).state.to_dictionary()
			TestAssertions.equal(actual_value, expected_value, "nested recovery preserves canonical legacy field %s" % field, failures)
...
func _test_schema_five_color_round_trip_and_validation(failures: Array[String]) -> void:
	...
	TestAssertions.equal(document.get("schema_version"), 6, "preferred colors persist in schema six", failures)
...
func _test_current_schema_reports_source_metadata(failures: Array[String]) -> void:
	...
	TestAssertions.equal(decoded.source_schema_version, 6, "current profile reports schema-six source", failures)
...
func _test_normal_store_is_current_only(failures: Array[String]) -> void:
	...
	TestAssertions.equal(loaded.source_schema_version, 6, "ordinary current store load reports schema-six source", failures)
...
static func _expected_schema_five_document(legacy: Dictionary) -> Dictionary:
	return _expected_schema_six_from_five(_expected_schema_five_from_four(_expected_schema_four_from_three(_expected_schema_three_from_two(_expected_schema_two_document(legacy)))))

static func _expected_schema_six_from_five(schema_five: Dictionary) -> Dictionary:
	var result := schema_five.duplicate(true)
	result["schema_version"] = 6
	result["terminal_resolution"] = {}
	result["terminal_recovery_overflow"] = ProfileState._empty_terminal_recovery_overflow(String(result["profile_id"]))
	for transaction_id: Variant in result["applied_transactions"] as Dictionary:
		var record := (result["applied_transactions"] as Dictionary)[transaction_id] as Dictionary
		record["result_profile"] = _expected_schema_six_from_five(record["result_profile"] as Dictionary)
	return result
