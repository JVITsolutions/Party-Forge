...
func _test_provider_result_is_immutable(failures: Array[String]) -> void:
	...
	result.set("diagnostic", &"mutated")
	result.set("snapshot", null)
	TestAssertions.equal([result.get("mode"), result.get("diagnostic")], [CANDIDATE, &""], "provider result mode and diagnostic are read only", failures)
	TestAssertions.truthy(result.get("snapshot") is CityAccessSnapshot, "provider result retains its candidate snapshot", failures)
