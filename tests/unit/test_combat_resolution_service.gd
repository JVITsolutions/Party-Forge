...
func run() -> Array[String]:
	...
	_test_fractional_block_range_rejects_later_life_steal_overflow(service_script, types, failures)
	_test_fractional_dodge_range_rejects_later_life_steal_overflow(service_script, types, failures)
...
func _test_fractional_block_range_rejects_later_life_steal_overflow(service_script: Script, types: DamageTypeCatalog, failures: Array[String]) -> void:
	var source_health := _health(1.0e308, 1.0)
	var source := _adapter(&"party:fractional_block_range", 1, source_health, {})
	var packet := _direct_packet(source, 4.0e307, 2.25, [false, true], 3.0)
	var target_health := _health(9.0e307, 9.0e307)
	var target := _adapter(&"enemy:fractional_block_range", 2, target_health, {&"block_chance": 0.5, &"block_effectiveness": 0.5})
	var rng := CombatRng.new(5503, [0.1, 0.9])
	var service: Node = service_script.new(rng, types)
	var publications := _publication_counts(service)
	var bundle: Object = service.call(&"resolve_bundle", packet, target)
	_assert_preflight_failure(bundle, target_health, 9.0e307, rng, publications, 1, "fractional-block later life-steal overflow", failures)
	TestAssertions.near(source_health.current_health, 1.0, 0.0, "fractional-block overflow fails before source life steal", failures)
	TestAssertions.equal(service.get("overkill_buffer").call(&"size"), 0, "fractional-block overflow leaves buffer empty", failures)
	service.free()

func _test_fractional_dodge_range_rejects_later_life_steal_overflow(service_script: Script, types: DamageTypeCatalog, failures: Array[String]) -> void:
	var source_health := _health(1.0e308, 1.0)
	var source := _adapter(&"party:fractional_dodge_range", 1, source_health, {})
	var packet := _direct_packet(source, 4.0e307, 2.25, [false, true], 2.0)
	var target_health := _health(9.0e307, 9.0e307)
	var target := _adapter(&"enemy:fractional_dodge_range", 2, target_health, {&"dodge_chance": 0.5})
	var rng := CombatRng.new(5504, [0.1, 0.9])
	var service: Node = service_script.new(rng, types)
	var publications := _publication_counts(service)
	var bundle: Object = service.call(&"resolve_bundle", packet, target)
	_assert_preflight_failure(bundle, target_health, 9.0e307, rng, publications, 1, "fractional-dodge later life-steal overflow", failures)
	TestAssertions.near(source_health.current_health, 1.0, 0.0, "fractional-dodge overflow fails before source life steal", failures)
	TestAssertions.equal(service.get("overkill_buffer").call(&"size"), 0, "fractional-dodge overflow leaves buffer empty", failures)
	service.free()
